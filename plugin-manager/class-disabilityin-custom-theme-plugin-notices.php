...
if ( ! class_exists( 'DisabilityIN_Custom_Theme_Plugin_Notices' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class DisabilityIN_Custom_Theme_Plugin_Notices {
		...
		public function set_notices() {
			...
			// DEBUG:
			// delete_user_meta( get_current_user_id(), 'din-dismiss-plugin-notice_update' );
			// delete_user_meta( get_current_user_id(), 'din-dismiss-plugin-notice_install' );
			...
			if ( $to_install ) {
				...
				if ( get_user_meta( get_current_user_id(), 'din-dismiss-plugin-notice_install', true ) !== $check ) {
			...
			if ( $to_update ) {
				...
				if ( get_user_meta( get_current_user_id(), 'din-dismiss-plugin-notice_update', true ) !== $check ) {
		...
		public function display( $key, $value, $message ) {
			...
			?>
			<div id="din-plugin-<?php echo esc_attr( $key ); ?>-notice" class="<?php echo $class; ?>" data-notice-key="<?php echo esc_attr( $key ); ?>" data-notice-value="<?php echo esc_attr( $value ); ?>" data-security="<?php echo esc_attr( $nonce ); ?>" data-namespace="din" data-dismiss="<?php echo esc_attr( $this->args['nag_dismiss'] ); ?>">
		...
		public function dismiss() {
			...
			update_user_meta(
				get_current_user_id(),
				'din-dismiss-plugin-notice_' . $_POST['notice_key'],
				$_POST['notice_value']
			);
